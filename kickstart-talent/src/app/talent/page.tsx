...
export default async function TalentPage({ searchParams }: TalentPageProps) {
  ...
  const savedStudents = (savedData as any)?.map((s: any) => s.student_id) || []
  ...
  if (search) {
    query = query.or(`headline.ilike.%${search}%,looking_for.ilike.%${search}%`)
  }

  const { data: students } = await query

  // Get unique skills for filter (from a sample of students)
    ...
    <DashboardLayout user={userData}>
      <div className="container py-8">
        ...
        <div className="max-w-3xl mb-8">
          <h1 className="text-3xl font-bold mb-2 text-foreground">Browse Talent</h1>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <form className="flex-1 relative">
            ...
            <Input
              name="q"
              placeholder="Search by headline, skills, or interests..."
              defaultValue={search}
              className="pl-10"
            />
          </form>
        ...
        {popularSkills && (popularSkills as any).length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            {(popularSkills as any).slice(0, 10).map((skill: any) => (
              <Link
                key={(skill as any).id}
                href={`/talent?skill=${(skill as any).name}`}
              >
                <Badge
          ...
          <>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(students as any).map((student: any) => {
                const profile = (student as any).profiles as {
                  id: string
                  full_name?: string
                  avatar_url?: string
                } | null
            ...
            action={
              (search || skillFilter) && (
                <Button asChild variant="outline">
                  <Link href="/talent">Clear filters</Link>
                </Button>
              )
            }
          />
